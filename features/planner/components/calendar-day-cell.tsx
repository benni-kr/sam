      ...
      className={`group relative h-full min-h-28 border-b border-r px-2 py-2 transition-colors last:border-r-0 ${
        isOver
          ...
          : // Weekend columns use a slightly darker tone.
            isWeekend
            ? "border-slate-200 bg-slate-50/90 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800/60 dark:hover:bg-slate-800"
            : "border-slate-200 bg-white hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:hover:bg-slate-800"
      }`}
    >
      <div className="text-[11px] font-medium text-slate-500 dark:text-slate-400">{day}</div>

      <button
        type="button"
        onClick={() => openCreateEvent(dateKey)}
        aria-label={`Create event on ${dateKey}`}
        className="absolute right-1.5 top-1.5 inline-flex h-6 w-6 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-500 opacity-0 shadow-sm transition-all hover:border-slate-300 hover:bg-slate-50 hover:text-slate-700 group-hover:opacity-100 group-focus-within:opacity-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400 dark:hover:border-slate-500 dark:hover:bg-slate-700 dark:hover:text-slate-200"
      >
